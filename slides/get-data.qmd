...
keeptabs <- acsvars2 |> 
  filter(table %in% c("B01003", "B01002", "B25008", "B19013", "B25071")) |> 
  select(table, concept, numvars) |> 
  distinct()
...
f <- function(tab, geo){
  print(tab)
  get_acs(geography = geo, table = tab, state = c("NY", "VT"), year = 2023)
}

states <- purrr::map(keeptabs$table, \(x) f(x, geo = "state")) |> 
  list_rbind() |> 
  mutate(geotype = "state")

counties <- purrr::map(keeptabs$table, \(x) f(x, geo = "county")) |> 
  list_rbind() |> 
  mutate(geotype = "county")

places <- purrr::map(keeptabs$table, \(x) f(x, geo = "place")) |> 
  list_rbind() |> 
  mutate(geotype = "place")

acs_data <- bind_rows(states, counties, places) |> 
  ...
  left_join(acs_variables |> 
            ...
            by = join_by(variable)) |> 
  mutate(trimname = str_remove(geoname, ", New York"),
         trimname = str_remove(trimname, ", Vermont"),
         trimname = case_when(
           geotype == "county" ~ str_remove(trimname, " County"),
           # geotype == "place" ~ str_remove(trimname, " village"),
           .default = trimname),
         trimname = ifelse(geotype == "county" & stabbr == "VT",
                           paste0(trimname, ", VT"),
                           trimname))

check <- count(acs_data, geoid, geotype, geoname, trimname)
